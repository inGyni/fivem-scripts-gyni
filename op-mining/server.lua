...
QBCore.Functions.CreateCallback('op-mining:get', function(source, cb, item, tablenumber, craftable)
	local hasitem = false
...
RegisterServerEvent('op-mining:GetItem', function(ItemMake, tablenumber, craftable)
	local src = source
...
RegisterServerEvent('op-mining:MineReward')
AddEventHandler('op-mining:MineReward', function()
...
RegisterServerEvent('op-mining:CrackReward')
AddEventHandler('op-mining:CrackReward', function()
    local Player = QBCore.Functions.GetPlayer(source)
    ...
    local amountToGive = math.random(2, 4)
    Player.Functions.AddItem(Config.RewardPool[oreToGive], amountToGive, false, {["quality"] = nil})
    TriggerClientEvent("inventory:client:ItemBox", source, QBCore.Shared.Items[Config.RewardPool[oreToGive]], "add", amountToGive)
end)

RegisterNetEvent("op-mining:Selling")
AddEventHandler("op-mining:Selling", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent("op-mining:SellJewel")
AddEventHandler("op-mining:SellJewel", function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local currentitem = data
    if Player.Functions.GetItemByName(data) ~= nil then
...
QBCore.Functions.CreateCallback('op-mining:Cutting:Check:Tools', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
